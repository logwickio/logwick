...
export default function Dashboard() {
  ...
  const S = {
    ...
    navBrand: { fontSize: 15, fontFamily: 'var(--font-sans)', fontWeight: 800, color: '#e2edf5', letterSpacing: '0.04em' },
    navSub: { fontSize: 9, color: '#1a3040', letterSpacing: '0.14em', textTransform: 'uppercase' },
    navItem: (active) => ({ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 18px', cursor: 'pointer', background: active ? '#0d1f2c' : 'transparent', borderLeft: active ? '2px solid #0ea5e9' : '2px solid transparent', color: active ? '#e2edf5' : '#2a4555', fontSize: 11, transition: 'all 0.15s', userSelect: 'none' }),
    ...
    pageTitle: { fontSize: 17, fontFamily: 'var(--font-sans)', fontWeight: 700, color: '#e2edf5' },
    ...
    statVal: { fontSize: 22, fontWeight: 800, fontFamily: 'var(--font-sans)', color: '#e2edf5', letterSpacing: '-0.02em', lineHeight: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
    ...
    searchBox: { flex: 1, minWidth: 140, background: '#0a1820', border: '1px solid #0e1c26', borderRadius: 6, padding: '7px 12px', color: '#e2edf5', fontSize: 11, outline: 'none' },
    sel: { background: '#0a1820', border: '1px solid #0e1c26', borderRadius: 6, padding: '7px 10px', color: '#e2edf5', fontSize: 11, outline: 'none', cursor: 'pointer' },
    ...
    blockBody: { padding: '12px 14px', fontSize: 11, color: '#e2edf5', lineHeight: 1.75, whiteSpace: 'pre-wrap', wordBreak: 'break-word' },
    ...
    metaV: { fontSize: 11, color: '#e2edf5' },
    ...
    fInput: { width: '100%', background: '#07101a', border: '1px solid #0e1c26', borderRadius: 6, padding: '8px 12px', color: '#e2edf5', fontSize: 11, outline: 'none' },
    fSel: { width: '100%', background: '#07101a', border: '1px solid #0e1c26', borderRadius: 6, padding: '8px 12px', color: '#e2edf5', fontSize: 11, outline: 'none', cursor: 'pointer' },
    fTA: { width: '100%', background: '#07101a', border: '1px solid #0e1c26', borderRadius: 6, padding: '8px 12px', color: '#e2edf5', fontSize: 11, outline: 'none', minHeight: 70, resize: 'vertical' },
  }
  ...
  function btn(variant = 'default', extra = {}) {
    return {
      ...
      color: variant === 'primary' ? '#fff' : variant === 'danger' ? '#ef4444' : '#e2edf5',
      ...extra,
    }
  }
        ...
        <div style={S.statsRow}>
          {[
            ...
            <div key={i} style={S.statCell(i === arr.length - 1)}>
              ...
              <div style={{ ...S.statVal, color: s.accent || '#e2edf5' }}>{s.value}</div>
          ...
          <div style={S.logPanel}>
            ...
            <div style={S.countBar}>
              ...
              {(search || fStatus !== 'all' || fAgent !== 'all' || fAction !== 'all' || dateFrom || dateTo) && (
                <span style={{ cursor: 'pointer', color: '#2a4555' }} onClick={() => { setSearch(''); setFStatus('all'); setFAgent('all'); setFAction('all'); setDateFrom(''); setDateTo('') }}>
                  × clear filters
                </span>
              )}
            </div>
            ...
            <div style={S.logList}>
              {logs.length === 0 && !logsLoading ? (
                <div style={{ padding: 48, textAlign: 'center', color: '#1a3040', fontSize: 12 }}>
                ...
                <div key={log.id} style={S.logRow(selected?.id === log.id)}
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 10, color: '#243545', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100%', marginBottom: 3 }}>
                        {log.input}
                      </div>
          ...
          {selected ? (
            <div style={S.detail}>
              <div style={{ padding: '11px 14px', borderBottom: '1px solid #0e1c26', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#07101a', flexShrink: 0 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#e2edf5', fontFamily: 'var(--font-sans)' }}>{selected.action}</span>
              ...
              <div style={S.detailScroll}>
                ...
                {selected.tags?.length > 0 && (
                  <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 10 }}>
                    {selected.tags.map(t => (
                      <span key={t} style={{ fontSize: 9, padding: '2px 7px', borderRadius: 4, background: '#0a1820', color: '#2a4555', border: '1px solid #0e1c26' }}>#{t}</span>
                    ))}
                  </div>
                )}
            ...
            <div style={S.sidebar}>
              ...
              {stats?.monthly_limit && (
                <div>
                  ...
                  <div style={{ fontSize: 11, color: '#2a4555', marginBottom: 6 }}>
    ...
    return (
      <div style={S.scrollContent}>
        <div style={{ maxWidth: 680 }}>
          <div style={{ fontSize: 20, fontFamily: 'var(--font-sans)', fontWeight: 800, color: '#e2edf5', marginBottom: 6 }}>API Reference</div>
          <div style={{ fontSize: 11, color: '#2a4555', marginBottom: 24, lineHeight: 1.7 }}>
            ...
            Auth: <code style={{ color: '#e2edf5' }}>Authorization: Bearer sk-lw-...</code>
          </div>

          {/* Keys */}
          <div style={{ ...S.block2 }}>
            <div style={{ padding: '12px 16px', borderBottom: '1px solid #0a1820', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: '#e2edf5', fontFamily: 'var(--font-sans)' }}>API Keys</span>
            ...
            {apiKeys.map(k => (
              <div key={k.id} style={{ padding: '12px 16px', borderBottom: '1px solid #0a1820', display: 'flex', gap: 12, alignItems: 'center' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#e2edf5', fontFamily: 'var(--font-sans)', marginBottom: 3 }}>{k.name}</div>
          ...
          <div style={{...S.block2, marginBottom: 24, border: '1px solid #0ea5e9', background: '#060e16'}}>
            <div style={{ padding: '12px 16px', borderBottom: '1px solid #0a1820', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: '#e2edf5', fontFamily: 'var(--font-sans)' }}>Connect to Claude Desktop</span>
              </div>
  ...
  "mcpServers": {
    "logwick": {
      ...
      "env": { "LOGWICK_API_KEY": "${apiKeys[0]?.key_prefix || 'sk-lw-your-key'}..." }
            ...
            <pre style={{ padding: '16px', fontSize: 11, color: '#e2edf5', lineHeight: 1.75, overflowX: 'auto', whiteSpace: 'pre' }}>{`{
  "mcpServers": {
    "logwick": {
      "command": "npx",
      "args": ["-y", "@logwick/mcp"],
            ...
            <div key={i} style={{ ...S.block2 }}>
              <div style={{ padding: '10px 16px', borderBottom: '1px solid #0a1820', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  ...
                  <code style={{ fontSize: 11, color: '#e2edf5' }}>{ep.path}</code>
                  <span style={{ fontSize: 11, fontWeight: 700, color: '#e2edf5', fontFamily: 'var(--font-sans)' }}>{ep.title}</span>
                </div>
                <button style={btn()} onClick={() => { navigator.clipboard.writeText(ep.code); toast('Copied') }}>Copy</button>
              </div>
              <pre style={{ padding: '16px', fontSize: 10, color: '#e2edf5', lineHeight: 1.7, overflowX: 'auto', whiteSpace: 'pre' }}>{ep.code}</pre>
            </div>
          ))}
        </div>
      </div>
    ...
    return (
      <div style={S.scrollContent}>
        <div style={{ maxWidth: 600 }}>
          <div style={{ fontSize: 20, fontFamily: 'var(--font-sans)', fontWeight: 800, color: '#e2edf5', marginBottom: 6 }}>Webhooks</div>
          ...
          <div style={{ ...S.block2, padding: 16, marginBottom: 20 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#e2edf5', fontFamily: 'var(--font-sans)', marginBottom: 12 }}>Add Webhook</div>
              ...
              <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#e2edf5', fontFamily: 'var(--font-sans)' }}>{w.label}</span>
                    ...
                    {(w.events || []).map(ev => <span key={ev} style={{ fontSize: 9, padding: '2px 6px', borderRadius: 4, background: '#0a1820', color: '#e2edf5', border: '1px solid #1c2e3a' }}>on:{ev}</span>)}
                  </div>
          ...
          {webhooks.length === 0 && <div style={{ fontSize: 11, color: '#1a3040', padding: '20px 0' }}>No webhooks yet. Add one above.</div>}
          ...
          <div style={{ ...S.block2, marginTop: 20 }}>
            ...
            <pre style={{ padding: 16, fontSize: 10, color: '#e2edf5', lineHeight: 1.7 }}>{`{
  "event":     "log.error",
    ...
    return (
      <div style={S.scrollContent}>
        <div style={{ maxWidth: 520 }}>
          <div style={{ fontSize: 20, fontFamily: 'var(--font-sans)', fontWeight: 800, color: '#e2edf5', marginBottom: 24 }}>Settings</div>
          {[
            ...
            <div key={sec.section} style={{ ...S.block2, marginBottom: 14 }}>
              <div style={{ padding: '9px 16px', borderBottom: '1px solid #0a1820', fontSize: 9, color: '#1a3040', letterSpacing: '0.12em', textTransform: 'uppercase' }}>{sec.section}</div>
          ...
          <div style={{ ...S.block2, marginBottom: 20 }}>
            ...
            <div style={{ padding: 16 }}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
                <span style={{ fontSize: 14, fontWeight: 800, color: '#e2edf5', fontFamily: 'var(--font-sans)', textTransform: 'uppercase' }}>{stats?.org_plan || 'free'}</span>
