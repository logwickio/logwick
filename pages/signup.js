...
export default function Signup() {
  const [step, setStep]         = useState('form')  // form | verify | onboard
  ...
  async function handleSignup(e) {
    ...
    // 2. Provision org via API (uses service role)
    ...
    // 3. Show API key (shown only once)
  ...
  if (step === 'onboard') return (
    <div className="auth-shell">
      <div className="auth-card fade-up" style={{ maxWidth: 480 }}>
        ...
        <div style={{ fontSize: 22, fontFamily: 'var(--font-sans)', fontWeight: 800, color: 'var(--bright)', marginBottom: 8 }}>
          You're in. 🎉
        </div>
        ...
        <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, padding: '14px 16px', marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--accent)', fontFamily: 'var(--font-mono)', wordBreak: 'break-all', lineHeight: 1.6 }}>
            {apiKey}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
          ...
          <a href="/dashboard" className="btn" style={{ flex: 1, justifyContent: 'center' }}>
            Go to Dashboard →
          </a>
        </div>

        <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, padding: '14px 16px' }}>
          <div style={{ fontSize: 9, color: 'var(--dim)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 10 }}>Quick start</div>
          <pre style={{ fontSize: 10, color: '#6a9ab5', lineHeight: 1.75, whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
{`curl -X POST https://api.logwick.io/v1/logs \\
  -H "Authorization: Bearer ${apiKey.slice(0, 20)}..." \\
  -H "Content-Type: application/json" \\
  -d '{"agent":"gpt-4o","action":"test","status":"success","input":"hello","output":"world"}'`}
          </pre>
        </div>
      </div>
    </div>
