...
export async function verifyX402Payment(req) {
  ...
  try {
    ...
    // Extract payer wallet address from payment proof
    let walletAddress = null
    try {
      const paymentData = JSON.parse(
        Buffer.from(paymentHeader.split('.')[0], 'base64').toString()
      )
      walletAddress = paymentData.from || paymentData.sender || paymentData.payerAddress || null
    } catch (e) {
      // Payment format varies — wallet extraction is best effort
    }

    return { valid: true, txHash: settleData.transaction, walletAddress }
